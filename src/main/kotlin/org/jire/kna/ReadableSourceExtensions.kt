...
fun ReadableSource.long(address: Long, offset: Long = 0) =
	read(address, Long.SIZE_BYTES.toLong())?.getLong(offset) ?: 0

fun ReadableSource.float(address: Long, offset: Long = 0) =
	read(address, Float.SIZE_BYTES.toLong())?.getFloat(offset) ?: 0

fun ReadableSource.double(address: Long, offset: Long = 0) =
	read(address, Double.SIZE_BYTES.toLong())?.getDouble(offset) ?: 0

fun ReadableSource.char(address: Long, offset: Long = 0) =
	read(address, Char.SIZE_BYTES.toLong())?.getChar(offset) ?: 0
